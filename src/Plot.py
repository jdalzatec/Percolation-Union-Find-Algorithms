...
def plot_lattice_percolation(
    lattice,
    labels=False,
    coordinates=False,
    colors={"open": "lightblue", "closed": "blue", "top": "pink"},
):
    ...
    for i, j in product(range(lattice.L), repeat=2):
        ...
        y = lattice.L - j - 1
        sq = Rectangle(
            (x + 0.05, y + 0.05), 0.9, 0.9, facecolor=color, edgecolor="black", lw=2
        )
        if labels:
            if coordinates:
                ax.text(
